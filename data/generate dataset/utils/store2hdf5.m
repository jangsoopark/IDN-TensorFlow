function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/gt', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
      startloc.lr=[ones(1,length(dat_dims)-1), 1];
      startloc.gt=[ones(1,length(lab_dims)-1), 1];
    end 
  else  % append mode
    if ~exist('startloc','var')
      ...
      startloc.lr=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
      startloc.gt=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
    end
  end

  if ~isempty(data)
    h5write(filename, '/lr', single(data), startloc.lr, size(data));
    h5write(filename, '/gt', single(labels), startloc.gt, size(labels));  
  end
